...
var scene,
    camera, fieldOfView, aspectRatio, nearPusheen, farPusheen,
    renderer, container;
...
function createScene() {
    ...
    renderer = new THREE.WebGLRenderer({ 
		...
		// but, as this project is low-poly based, it should be fine
        antialias: true 
    });
...
function createLights() {
    ...
    hemisphereLight = new THREE.HemisphereLight(0xaaaaaa, 0xaaaaaa, .9);
   
    // an ambient light modifies the global color of a scene and makes the shadows softer
    ambientLight = new THREE.AmbientLight(0x114271, .2);
...
var Pusheen = function () {
    ...
    var hair1Mat = new THREE.MeshLambertMaterial({ color: Colors.red });
...
Pusheen.prototype.updateHairs = function () {
   ...
   // get the hair
    ...
    for (var i = 0; i < l; i++) {
        ...
        h1.scale.y = .75 + Math.cos(this.angleHairs + i / 8) * .25;
        h2.scale.y = .75 + Math.cos(this.angleHairs + i / 8) * .25;
        h3.scale.y = .75 + Math.cos(this.angleHairs + i / 8) * .25;
    }
...
Tail.prototype.updateHairs = function () {
   ...
   // get the hair
    ...
    for (var i = 0; i < l; i++) {
        ...
        var h3 = hairs3[i];
        // each hair element will scale on cyclical basis between 75% and 100% of its original size
        h1.scale.y = .60 + Math.cos(this.angleHairs + i / 8) * .25;
        h2.scale.y = .60 + Math.cos(this.angleHairs + i / 8) * .25;
        h3.scale.y = .60 + Math.cos(this.angleHairs + i / 8) * .25;
    }
    // increment the angle for the next frame
    this.angleHairs += 0.12;
}
...
var Body = function () {
	...
	var geomCockpit = new THREE.ExtrudeGeometry( starShape, {amount: 30, bevelEnabled: false} );
	...
	var matHorn = new THREE.MeshPhongMaterial({ color: Colors.white, shading: THREE.FlatShading });
	...
	var matEye = new THREE.MeshPhongMaterial({ color: Colors.darkgrey, shading: THREE.FlatShading });
	...
	var matWhisker = new THREE.MeshPhongMaterial({ color: Colors.darkgrey, shading: THREE.FlatShading });
	...
	var geomMouth = new THREE.ExtrudeGeometry( mouthShape, {amount: 1, bevelEnabled: false} );
	var matMouth = new THREE.MeshPhongMaterial({ color: Colors.darkgrey, shading: THREE.FlatShading });
...
function init(event) {
    ...
    // set up the scene, the camera and the renderer, lights and objects
    createScene();
    createLights();
    createPusheen();
    createSea();
